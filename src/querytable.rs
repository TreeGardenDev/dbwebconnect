...
pub fn query_tables(table: &str, conn: &mut PooledConn, whereclause: &str, database: &str)->Vec<String>{
...
fn query_table(conn: &mut PooledConn, table: &str, whereclause: &str, database: &str, columntypes: Vec<String>) -> std::result::Result<Vec<String>, Box<dyn std::error::Error>> {
    let mut query= String::from("SELECT * FROM ");
    query.push_str(table);
    if whereclause != "" {
